...
function getChatCompletionText(response: any) {
  ...
  if (typeof text !== "string" || text.trim().length === 0) {
    throw new Error(`Groq response did not include choices[0].message.content: ${JSON.stringify(response)}`);
  }
...
function isProviderStopError(error: unknown) {
  ...
  return /429|rate.?limit|quota|credit|insufficient|temporarily unavailable|overloaded|resourceexhausted|too many requests/i.test(
    message
  );
}
