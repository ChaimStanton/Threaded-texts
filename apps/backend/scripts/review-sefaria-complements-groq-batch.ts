...
const completionWindow = args.get("completion-window") ?? "7d";
const maxCompletionTokens = Number(args.get("max-completion-tokens") ?? 700);
...
if (!env.GROQ_API_KEY) {
  throw new Error("GROQ_API_KEY is required to review Sefaria complements with Groq.");
}
...
function extractChatCompletionText(body: any) {
...
async function submitBatch() {
  ...
  const rows = await prisma.textSefariaComplement.findMany({
    where: {
      deletedAt: null,
  ...
  const jsonlPath = join(tmpdir(), "lsjs-sacks-groq-batches", `sefaria-complement-reviews-${Date.now()}.jsonl`);
  const requests = [];
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a strict Jewish source QA reviewer. Return only JSON. Judge fit, not plausibility."
        },
        {
          role: "user",
...
async function importBatch(batchIdToImport: string) {
  const batch = await groq.batches.retrieve(batchIdToImport);
...
async function pollBatch(batchIdToPoll: string) {
  ...
  while (Date.now() - startedAt <= maxWaitMs) {
    const batch = await groq.batches.retrieve(batchIdToPoll);
    console.log(
      JSON.stringify({
        batchId: batch.id,
        status: batch.status,
    ...
    if (["failed", "expired", "cancelled"].includes(String(batch.status))) {
      return;
    }
